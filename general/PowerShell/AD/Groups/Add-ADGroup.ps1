...
function main() {
  ...
  foreach ($companyLine in $csvFile) {
    ...
    $thisGroup = New-ADGroup -Path $companyLine.OUDN -Name $companyLine.GroupName -DisplayName $companyLine.GroupName -SamAccountName $companyLine.SamAccountName -Description $companyLine.GroupDescription -GroupCategory $companyLine.GroupType -GroupScope $companyLine.GroupScope

    $thisGroup | Set-ADGroup -Replace @{info="$($companyLine.Notes)"}
  }
